#!/bin/bash
# Robot 1 실행 스크립트 (Domain 1, IGN_PARTITION=robot1)
# gnome-terminal 탭 4개로 rl1~rl4 자동 실행
...
gnome-terminal --window \
    --tab --title="R${ROBOT_ID}-rl1" -e "bash -c 'export ROS_DOMAIN_ID=$DOMAIN_ID; export IGN_PARTITION=$IGN_PART; cd $ROS2_WS; source install/setup.bash; echo rl1: Gazebo + Robot; ros2 launch agilex_scout simulate_control_gazebo.launch.py lidar_type:=3d rviz:=true gui:=false; exec bash'" \
